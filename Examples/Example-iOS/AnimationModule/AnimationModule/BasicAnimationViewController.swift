...
//        }
    ...
    private func playAnimationSequence() {
        let animations: [() -> Void] = [
            fadeInTapped,
            { DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) { self.scaleTapped() } },
            { DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) { self.rotateTapped() } },
            { DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) { self.bounceTapped() } },
            { DispatchQueue.main.asyncAfter(deadline: .now() + 4.0) { self.resetTapped() } }
        ]
