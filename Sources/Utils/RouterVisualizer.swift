    ...
    private func generateDotRouteTree(_ routes: [(pattern: String, type: String)]) -> String {
        ...
        for route in routes {
            ...
            for component in components {
                ...
                guard let targetNode = nodeMap[currentPath] else { continue }
                dot += "    \(currentParent) -> \(targetNode);\n"
    ...
    private func generateMermaidModuleGraph(_ modules: [String]) -> String {
        ...
        if modules.count > 1 {
            for index in 0..<modules.count-1 {
                let from = modules[index].replacingOccurrences(of: " ", with: "_")
                let to = modules[index+1].replacingOccurrences(of: " ", with: "_")
                mermaid += "    \(from) --> \(to)\n"
            }
        }
