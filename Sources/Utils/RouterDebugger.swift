...
public actor RouterDebugger {
    ...
    private func calculateSimilarity(_ str1: String, _ str2: String) -> Double {
        ...
        _ = str1.count > str2.count ? str2 : str1
    ...
    private func levenshteinDistance(_ str1: String, _ str2: String) -> Int {
        ...
        for row in 0...str1Count {
            matrix[row][0] = row
        }

        for col in 0...str2Count {
            matrix[0][col] = col
        }

        for row in 1...str1Count {
            for col in 1...str2Count {
                if str1Array[row-1] == str2Array[col-1] {
                    matrix[row][col] = matrix[row-1][col-1]
                } else {
                    matrix[row][col] = min(
                        matrix[row-1][col] + 1,    // 删除
                        matrix[row][col-1] + 1,    // 插入
                        matrix[row-1][col-1] + 1   // 替换
                    )
                }
            }
        }
